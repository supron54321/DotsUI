...
namespace DotsUI.Hybrid
{
    ...
    class UnityEngineInputHandler : ComponentSystem
    {
        ...
        private ControlsInputSystem m_ControlsInputSystem;

        protected override void OnCreate()
        {
            m_LastFrameMousePos = (Vector2) UnityEngine.Input.mousePosition;
            m_ControlsInputSystem = World.GetOrCreateSystem<ControlsInputSystem>();
        }
        ...
        protected override void OnUpdate()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                if (UnityEngine.Input.GetMouseButtonDown(i))
                    writer.AddPointerEvent(new NativePointerButtonEvent()
                    {
            ...
            // I had a problem with this loop, so I put this max loop count to avoid deadlocks. I'll investigate this later. Probably misunderstood Event documentation :)
            for (int i = 0; i < 100 && Event.PopEvent(m_Events); i++)
            {
                if (m_Events.type == EventType.KeyDown)
                {
                    writer.AddKeyboardEvent(new NativeKeyboardInputEvent()
                    {
